...
canvas.onclick = (event) => { const p = point(event); send({ type: "drop", x: p.x, y: p.y, color: selected }); };
window.addEventListener("keydown", (event) => { if (!me || !["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight", "w", "a", "s", "d", "W", "A", "S", "D"].includes(event.key)) return; event.preventDefault(); const step = .8; const x = me.x + ((event.key === "ArrowRight" || event.key.toLowerCase() === "d") ? step : (event.key === "ArrowLeft" || event.key.toLowerCase() === "a") ? -step : 0); const y = me.y + ((event.key === "ArrowDown" || event.key.toLowerCase() === "s") ? step : (event.key === "ArrowUp" || event.key.toLowerCase() === "w") ? -step : 0); const now = Date.now(); if (now - lastMove > 45) { lastMove = now; send({ type: "move", x, y }); } });
