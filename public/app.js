...
function syncDog(dog) { let mesh = dogMeshes.get(dog.id); if (!mesh || mesh.userData.breed !== dog.breed) { if (mesh) scene.remove(mesh); mesh = makeDog(dog); dogMeshes.set(dog.id, mesh); } mesh.position.set(dog.x, 0, dog.y); mesh.scale.setScalar(DOG_SCALES[dog.breed] || 1); mesh.userData.targetHeading = dog.heading ?? mesh.userData.targetHeading ?? 0; }
function syncNpc(npc) { let mesh = npcMeshes.get(npc.id); if (!mesh) { mesh = makeDog(npc, true); npcMeshes.set(npc.id, mesh); } mesh.position.set(npc.x, 0, npc.y); mesh.userData.targetHeading = npc.heading; }
...
function animate() { requestAnimationFrame(animate); const delta = Math.min(clock.getDelta(), .1); for (const direction of heldDirections) move(direction); if (me) { const target = new THREE.Vector3(me.x, 1, me.y); cameraTarget.lerp(target, 1 - Math.exp(-delta * 5)); const now = Date.now(); const toy = [...toys.values()].find(item => Math.hypot(item.x - me.x, item.y - me.y) < 1.35); const cookie = [...cookies.values()].find(item => Math.hypot(item.x - me.x, item.y - me.y) < 1.35); if (now - lastEat > 250 && (toy || cookie)) { lastEat = now; const item = toy || cookie; send({ type:"eat", itemId:item.id, itemType:toy ? "toy" : "cookie" }); } }
  ...
  const horizontal = Math.cos(cameraOrbit.elevation) * cameraOrbit.radius; const cameraPosition = new THREE.Vector3(cameraTarget.x + Math.sin(cameraOrbit.azimuth) * horizontal, cameraTarget.y + Math.sin(cameraOrbit.elevation) * cameraOrbit.radius, cameraTarget.z + Math.cos(cameraOrbit.azimuth) * horizontal); camera.position.lerp(cameraPosition, 1 - Math.exp(-delta * 8)); camera.lookAt(cameraTarget); for (const mesh of [...dogMeshes.values(), ...npcMeshes.values()]) { turnDog(mesh, delta); mesh.userData.label?.quaternion.copy(camera.quaternion); } for (const mesh of itemMeshes.values()) { mesh.rotation.y += delta * 1.8; if (mesh.userData.bob) mesh.position.y = Math.sin(clock.elapsedTime * 3 + mesh.userData.bob) * .07; } renderer.render(scene, camera); }
