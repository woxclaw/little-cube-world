...
function faceDog(direction) { const delta = ({ up:[0,-1], down:[0,1], left:[-1,0], right:[1,0] })[direction]; const mesh = me && dogMeshes.get(me.id); if (!delta || !mesh) return; const heading = Math.atan2(delta[1], delta[0]); mesh.userData.targetHeading = heading; mesh.rotation.y = -heading; }
function setHeldDirection(direction, active) { if (active) { heldDirections.add(direction); faceDog(direction); } else heldDirections.delete(direction); }
